...
#         user_id = request.user.id
#         banner = MyProduct(request.POST or None, request.FILES or None)
#
#         # newProduct = Products(name=name, description=description, quantity=quantity, price=price,
#         #                       accounts_account_id_id=user_id, category_id=category, banner=banner)
#
#         if banner.is_valid():
#             banner.save()
#         newProduct.save()
...
def addNewProduct(request):
    allCategories = Categories.objects.all()
    product = MyProduct(request.POST or None, request.FILES or None)
    accounts_account_id = request.POST.get('accounts_account_id')
    product.accounts_account_id = accounts_account_id
    user_id = request.user.id
